...
std::vector<uint8_t> Hmac::sha256(const std::vector<uint8_t>& data) {
  ...
  for (size_t chunk = 0; chunk < padded.size(); chunk += 64) {
    ...
    for (int i = 16; i < 64; i++) {
      uint32_t s0 =
          rightRotate32(w[i - 15], 7) ^ rightRotate32(w[i - 15], 18) ^ (w[i - 15] >> 3);
...
std::vector<uint8_t> Hmac::sha512(const std::vector<uint8_t>& data) {
  ...
  for (size_t chunk = 0; chunk < padded.size(); chunk += 128) {
    ...
    for (int i = 16; i < 80; i++) {
      uint64_t s0 =
          rightRotate64(w[i - 15], 1) ^ rightRotate64(w[i - 15], 8) ^ (w[i - 15] >> 7);
      uint64_t s1 =
          rightRotate64(w[i - 2], 19) ^ rightRotate64(w[i - 2], 61) ^ (w[i - 2] >> 6);
      w[i] = w[i - 16] + s0 + w[i - 7] + s1;
    }
